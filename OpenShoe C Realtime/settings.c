...
float gravity(float lambda, float h)
{
    ...
    float gamma = 9.780327 * (1 + 0.0053024 * (sin(lambda) * sin(lambda)) - 0.0000058 * (sin(2 * lambda) * sin(2 * lambda)));
    float g = gamma - ((3.0877e-6) - (0.004e-6) * (sin(lambda) * sin(lambda))) * h + (0.072e-12) * (h * h);
    return g;
}
...
void write_Output(float x[9][WINDOWS_SIZE]) {
  ...
  FILE *fp_output = fopen("result.txt", "a");
...
void write_Output_zupt(int zupt[WINDOWS_SIZE]) {
  ...
  FILE *fp_output = fopen("result_zupt_realtime.txt", "a");
