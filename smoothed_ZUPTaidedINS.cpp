...
void smoothed_ZUPTaidedINS(float u[][WINDOWS_SIZE], int zupt[], float x[][WINDOWS_SIZE], float cov_smoothed[][WINDOWS_SIZE], float P_smooth[][9][WINDOWS_SIZE], float dx[][WINDOWS_SIZE], float dx_smooth[][WINDOWS_SIZE], float P[][9][WINDOWS_SIZE])
{
    ...
    static float quat[4][WINDOWS_SIZE];
    memset(quat, 0, 4 * WINDOWS_SIZE * sizeof (float));

    //static float dx[9][WINDOWS_SIZE];
    //memset(dx, 0, 9 * WINDOWS_SIZE * sizeof (float));

    static float dx_timeupd[9][WINDOWS_SIZE];
    memset(dx_timeupd, 0, 9 * WINDOWS_SIZE * sizeof (float));

    //static float dx_smooth[9][WINDOWS_SIZE];
    //memset(dx_smooth, 0, 9 * WINDOWS_SIZE * sizeof (float));

    // Allocate covariance matrices
    static float cov[9][WINDOWS_SIZE];
    memset(cov, 0, 9 * WINDOWS_SIZE * sizeof (float));

    //static float P[9][9][WINDOWS_SIZE];
    //memset (P, 0, 9 * 9 * WINDOWS_SIZE * sizeof (float));

    static float P_timeupd[9][9][WINDOWS_SIZE];
    memset (P_timeupd, 0, 9 * 9 * WINDOWS_SIZE * sizeof (float));

    //static float P_smooth[9][9][WINDOWS_SIZE];
    //memset (P_smooth, 0, 9 * 9 * WINDOWS_SIZE * sizeof (float));

    static float Fn[9][9][WINDOWS_SIZE];
    memset (Fn, 0, 9 * 9 * WINDOWS_SIZE * sizeof (float));
    ...
    while(1) {
        ...
        for(n = seg_start; n <= seg_end; n++) {
            ...
            float u_h[6] = {u[0][n], u[1][n], u[2][n], u[3][n], u[4][n], u[5][n]};
            ...
            float sum = cov[3][n - 1] + cov[4][n - 1] + cov[5][n - 1];
            float sum_n = cov[3][n] + cov[4][n] + cov[5][n];
...
void inv_mat_9(float I[][9], float P_timeupd[][9][WINDOWS_SIZE], int n)
{
...
void update_deviation_estimate(float dx[][WINDOWS_SIZE], float K[][3], float x[][WINDOWS_SIZE], int n)
{
    ...
    float res1[3] = {dx[3][n] - x[3][n], dx[4][n] - x[4][n], dx[5][n] - x[5][n]};
